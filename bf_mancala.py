...
def solve_sungka(current_board_state, path=[]):
    """
    Finds the optimal sequence of moves to maximize seeds in store.
    """
    #num_pits = len(current_board_state) - 1
    #print(f"numpits: {num_pits}")
    #print(f"f: {current_board_state} try :{current_board_state[num_pits-1]}")
    best_score = current_board_state[num_pits]
    best_path = path
    #print(f"current_state: {current_board_state}, path: {path}")

    # Try every possible pit
    for pit_idx in range(num_pits):
        if current_board_state[pit_idx] > 0:
            board_obj = SungkaBoard(num_pits, current_board_state[:num_pits])
            board_obj.pits = list(current_board_state) # Sync state
            
            next_state, can_continue, game_over = board_obj.simulate_move(pit_idx)
            
            if next_state is not None:
                current_path = path + [pit_idx]
                
                if can_continue:
                    # Recursive search for the next move in the chain
                    score, final_path, last= solve_sungka(next_state, current_path)
                else:
                    # Game Over branch: record score
    ...
    return best_score, best_path, current_board_state
...
if __name__ == "__main__":
    ...
    output_file = "results/bf/results_pit_"+ str(num_pits) +".json"
